...
fun MainScreen() {
    ...
    var detailOpenCount by rememberSaveable { mutableIntStateOf(0) }
    ...
    val isHome = !showSettings && selectedTab == 0 && homeDetailValueCents == null && !showValueExamples && !showAmountBreakdown
                ...
                0 -> {
                    when {
                        ...
                        homeDetailValueCents != null -> {
                            ...
                            HomeTabContent(
                                onOpenCard = { valueCents ->
                                    detailOpenCount += 1
                                    if (isOnline && detailOpenCount % 3 == 0) {
                                        val activity = context as? Activity
                                        if (activity != null && interstitialAd.showIfReady(activity) {
                                            // After ad is dismissed, open the detail
                                            homeDetailValueCents = valueCents
                                        }) {
                                            // Ad was shown, detail will open after dismiss
                                        } else {
                                            // Ad not ready, open detail immediately
                                            homeDetailValueCents = valueCents
                                        }
                                    } else {
                                        homeDetailValueCents = valueCents
                                ...
                                onShowAmountBreakdown = { showAmountBreakdown = it },
                                modifier = Modifier.fillMaxSize()
                            )
                        }
                    }
